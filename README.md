...
If you don"t want to run all of the browser versions then you can use "one-version", but you have to change the `if statement` in the `main.py` file (`all_versions = cfg.get("browsers")[browserID].get("versions")` --> `all_versions = cfg.get("browsers")[browserID].get("one-version")`).
